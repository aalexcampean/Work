...
public class TaskMechanism
{
    ...
    private static void StartClient(string host, int taskID)
    {
        ...
        Send(state, HTTPUtilities.getRequestString(state.hostname, state.endpointPath)).Wait();
    ...
    private static void ReceiveCallback(IAsyncResult ar)
    {
        ...
        try
        {
            ...
            if (!HTTPUtilities.responseHeaderFullyObtained(state.responseContent.ToString()))
            {
            ...
            else
            {
                ...
                if (responseBody.Length < HTTPUtilities.getContentLength(state.responseContent.ToString()))
                {
                    ...
                    clientSocket.BeginReceive(state.receiveBuffer, 0, StateObject.BUFFER_SIZE, 0, ReceiveCallback,
                        state);
                }
                else
                {
