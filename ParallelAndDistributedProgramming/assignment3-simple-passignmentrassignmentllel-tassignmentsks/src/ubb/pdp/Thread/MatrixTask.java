...
public abstract class MatrixTask extends Thread {
    ...
    protected final Matrix A, B, resultMatrix;
    protected int K;

    public MatrixTask(int startRow, int startColumn, int elementCount, Matrix A, Matrix B, Matrix resultMatrix) {
    ...
    public void run() {
        for (AbstractMap.SimpleEntry<Integer, Integer> element : elements) {
            ...
            resultMatrix.set(row, column, Utils.computeMatrixElement(A, B, row, column));
        }
    }
}
