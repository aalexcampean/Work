...
//                }
    ...
    public void setLockTable() {
        if (myController.getRepo().getPrgList().size() > 0) {
            ...
            lockTableValue.setCellValueFactory(cellData -> new ReadOnlyStringWrapper(Integer.toString(cellData.getValue().getValue())));
            lockTableList.addAll(myController.getRepo().getPrgList().get(0).getLockTbl().getContent().getContent().entrySet());
