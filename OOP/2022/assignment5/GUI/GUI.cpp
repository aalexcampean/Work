...
GUI::GUI(AdminService *aService, UserService *uService, QWidget *parent) : QWidget(parent) {
...
void GUI::mapSignalsToSlots() {
    ...
    QObject::connect(this, SIGNAL(availableDogsUpdateSignal(QListWidget * , const std::vector<Dog>&)), this,
                     SLOT(populateList(QListWidget * , const std::vector<Dog>&)));
    QObject::connect(this, SIGNAL(adoptionListUpdateSignal(QListWidget * , const std::vector<Dog>&)), this,
                     SLOT(populateList(QListWidget * , const std::vector<Dog>&)));;
...
void GUI::adminMenu() {
    ...
    bottomLayout->addWidget(this->adminAddButton, 0, 0);
    bottomLayout->addWidget(this->adminUpdateButton, 0, 1);
    bottomLayout->addWidget(this->adminDeleteButton, 1, 0);
    bottomLayout->addWidget(this->adminCloseButton, 1, 1);
...
void GUI::userMenu() {
    ...
    this->userAvailableList = new QListWidget{};
    this->userAvailableList->setSelectionMode(QAbstractItemView::SingleSelection);
    this->populateList(this->userAvailableList, this->userAvailableVector = this->adminService->getRepository());
    ...
    topLayout->addWidget(this->userAvailableList);
...
void GUI::manageAdoptionList(std::pair<QWidget *, QLayout *> receivedWidgetLayout) {
    ...
    this->userAdoptionList = new QListWidget{};
    this->userAdoptionList->setSelectionMode(QAbstractItemView::SingleSelection);
    this->populateList(this->userAdoptionList, this->userAdoptionVector);
    ...
    receivedWidgetLayout.second->addWidget(this->userAdoptionList);
}
...
void GUI::filterListByBreed(const std::string &inputBreed) {
    ...
    emit this->availableDogsUpdateSignal(this->userAvailableList, filteredDogs);
}

void GUI::adoptDog() {
    int idx = this->getSelectedIndex(this->userAvailableList);
    ...
    emit availableDogsUpdateSignal(this->userAvailableList, this->userAvailableVector);
    emit adoptionListUpdateSignal(this->userAdoptionList, this->userAdoptionVector);
//    std::string url = std::string("open ").append(addDog->getTrailer());
//    system(url.c_str());
}

void GUI::dropAdoption() {
    int idx = this->getSelectedIndex(this->userAdoptionList);
    if (idx != -1) {
...
void GUI::addDog(std::vector<std::string> receivedFields, QLabel *errorMessage) {
    bool flag = true;
    try {
        this->adminService->addElementService(receivedFields[0], receivedFields[1], receivedFields[2],
                                              receivedFields[3]);
    }
...
void GUI::updateDog(std::vector<std::string> receivedFields, QLabel *errorMessage) {
    bool flag = true;
    try {
        this->adminService->updateElementService(receivedFields[0], receivedFields[1], receivedFields[2],
                                                 receivedFields[3]);
    }
    catch (const CustomException &e) {
...
void GUI::deleteDog(const std::string &receivedImageLink) {
    try {
        this->adminService->deleteElementService(receivedImageLink);
    }
    catch (const CustomException &e) {
    }
    emit this->dataBaseUpdate();
