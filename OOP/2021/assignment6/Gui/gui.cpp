...
GUI::GUI(QWidget *parent) : QWidget(parent) {
    ...
    this->op = new OperationExecution;
...
void GUI::populateUserList(std::vector<Movie> inputVector) {
    delete this->moviesTableModelUser;
    this->moviesTableModelUser = new MoviesTableModel(inputVector);
    this->moviesTableViewUser->setModel(this->moviesTableModelUser);
}

void GUI::populateWatchlist(std::vector<Movie> inputVector) {
    delete this->watchlistModel;
    this->watchlistModel = new MoviesTableModel(inputVector);
    this->watchlistView->setModel(this->watchlistModel);
}
...
void GUI::adminMenu() {
    ...
    this->a_undo = new QPushButton{ "Undo" };
    this->a_undo->setMinimumHeight(35);
    this->a_redo = new QPushButton{ "Redo" };
    this->a_redo->setMinimumHeight(35);
    ...
    QObject::connect(this->a_add, &QPushButton::clicked, this, [=](){
        ...
        this->a_undo->setDisabled(true);
        this->a_redo->setDisabled(true);
    ...
    QObject::connect(a_undo, &QPushButton::clicked, this, [=]() {
        emit this->undoSignal();
    });

    QObject::connect(a_redo, &QPushButton::clicked, this, [=]() {
    ...
    bottomLayout->addWidget(this->a_undo, 0, 0);
    bottomLayout->addWidget(this->a_redo, 0, 1);
    bottomLayout->addWidget(this->a_add, 1, 0);
    bottomLayout->addWidget(this->a_update, 1, 1);
    bottomLayout->addWidget(this->a_delete, 2, 0);
    bottomLayout->addWidget(this->a_close, 2, 1);
...
void GUI::userMenu() {
    ...
    this->moviesTableViewUser = new QTableView();
    this->moviesTableViewUser->setSelectionMode(QAbstractItemView::SingleSelection);
    this->moviesTableViewUser->setSelectionBehavior(QAbstractItemView::SelectRows);

    this->moviesVectorUser = this->as.getDataBase();
    this->moviesTableModelUser = new MoviesTableModel(this->moviesVectorUser);
    this->moviesTableViewUser->setModel(this->moviesTableModelUser);

//    this->populateUserLists(this->moviesTableUser, this->moviesVectorUser = this->as.getDataBase());
    ...
    topLayout->addWidget(this->moviesTableViewUser);
...
void GUI::addFunctionality(std::pair<QWidget*, QLayout*> receivedWidgetLayout) {
    ...
    QObject::connect(close, &QPushButton::clicked, this, [=]() {
        ...
        this->a_undo->setDisabled(false);
        this->a_redo->setDisabled(false);
    });
...
void GUI::filterList(const std::string& searchGenre) {
    ...
    emit this->moviesListUpdate(filteredMoviesVector);
}
...
void GUI::manageWatchList(std::pair<QWidget*, QLayout*> receivedWidgetLayout) {
    ...
    this->watchlistView->setSelectionMode(QAbstractItemView::SingleSelection);
    this->watchlistView->setSelectionBehavior(QAbstractItemView::SelectRows);

    this->watchlistVector = {};
    this->watchlistModel = new MoviesTableModel(this->watchlistVector);
    this->watchlistView->setModel(this->watchlistModel);
    ...
    QObject::connect(this->u_done, &QPushButton::clicked, this, [=](){
        ...
        delete this->watchlistView;
        delete this->watchlistModel;
        delete verticalButtonLayout;
        delete verticalButtonWidget;
        this->u_watchlist->setDisabled(false);
        this->us.openFile();
    });

    receivedWidgetLayout.second->addWidget(verticalButtonWidget);
    receivedWidgetLayout.second->addWidget(this->watchlistView);
}
...
void GUI::connectSignalsAndSlots() {
    ...
    QObject::connect(this, SIGNAL(moviesListUpdate(std::vector<Movie>)),this, SLOT(populateUserList(std::vector<Movie>)));
    QObject::connect(this, SIGNAL(watchListUpdate(std::vector<Movie>)), this, SLOT(populateWatchlist(std::vector<Movie>)));
    ;
...
void GUI::addMovie(std::vector<std::string> receivedFields, QLabel* errorMessage) {
    bool flag = true;
    try {
        Movie newMovie(receivedFields[0], receivedFields[1], std::stoi(receivedFields[2]), std::stoi(receivedFields[3]), receivedFields[4]);
        AddOperation* newOperation = new AddOperation(newMovie, this->as.getRepoState());
...
void GUI::updateMovie(std::vector<std::string> receivedFields, QLabel* errorMessage) {
    ...
    try {
        Movie newMovie(receivedFields[0], receivedFields[1], std::stoi(receivedFields[2]), std::stoi(receivedFields[3]), receivedFields[4]);
        Movie oldMovie = this->as.getElementByTitle(receivedFields[0]);
        UpdateOperation* newOperation = new UpdateOperation(oldMovie, newMovie, this->as.getRepoState());
        this->op->saveOperation(newOperation);
    }
    catch (const CustomException& e) {
...
void GUI::deleteMovie(const std::string& receivedTitle) {
    try {
        Movie oldMovie = this->as.getElementByTitle(receivedTitle);
        DeleteOperation* newOperation = new DeleteOperation(oldMovie, this->as.getRepoState());
        this->op->saveOperation(newOperation);
    }
    catch (const CustomException& e) {
    }
...
void GUI::addToWatchList() {
    int idx = this->moviesTableViewUser->selectionModel()->currentIndex().row();
    ...
    emit this->moviesListUpdate(this->moviesVectorUser);
    emit this->watchListUpdate(this->watchlistVector);
//    std::string url = std::string("open ").append(addMovie->getTrailer());
//    system(url.c_str());
}

void GUI::deleteFromWatchlist() {
    int idx = this->watchlistView->selectionModel()->currentIndex().row();
    if (idx != -1) {
        Movie* deleteMovie = &this->watchlistVector[idx];
        auto it = std::find(this->watchlistVector.begin(), this->watchlistVector.end(),deleteMovie->getTitle());
